...
def test_on_dataset():
  ...
  for row in dataset:
    ...
    assert expected_category.lower() in answer.lower(), f"expected: {expected_category}, got {answer}"
    if expected_subjects:
      assert any(subject.lower() in answer.lower() for subject in expected_subjects), f"Expected the assistant questions to include '{expected_subjects}', but got {answer}"
test_on_dataset()
